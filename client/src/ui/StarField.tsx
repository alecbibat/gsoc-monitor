...
export function StarField() {
  ...
  useEffect(() => {
    ...
    // Painted at the current viewport size, and repainted whenever it changes
    // (fullscreen, maximize, rotation) so the new area isn't left starless.
    // The RNGs are re-seeded on every paint, so each repaint is exactly what a
    // fresh load at that size would draw.
    const draw = () => {
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      const w = window.innerWidth;
      const h = window.innerHeight;
      canvas.width = Math.round(w * dpr); // resets bitmap + context state
      canvas.height = Math.round(h * dpr);
      canvas.style.width = `${w}px`;
      canvas.style.height = `${h}px`;
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0); // absolute, not a cumulative scale()
    ...
    // At most one repaint per frame, however fast resize events arrive.
    let raf = 0;
    const onResize = () => {
      if (raf) return;
