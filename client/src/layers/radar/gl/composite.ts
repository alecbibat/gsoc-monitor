...
import { compositeRegion, WORKER_SLOTS } from '../worker/pool';
...
export function planRegion(_viewer: Cesium.Viewer, maxLevel: number): CompositeRegion | null {
  ...
  const level = Math.min(maxLevel, observed[0].level);
  ...
  // the spike declines rather than stitching a block most of which is off
  // screen anyway.
